...
from .db import DEFAULT_DB_DIRECTORY_PATH, CREATE_TABLE_WORD
from .db import search_database, add_word, delete_word, set_priority, list_letter, list_latest, list_priority, super_insert, count_word
...
def main():
    ...
    if args.add:
        ...
        if input_file_path.endswith('.txt'):
            ...
            super_insert(os.path.join(DEFAULT_DB_DIRECTORY_PATH, 'word_list.txt'))
        else:
        ...
        if not os.path.exists(os.path.join(DEFAULT_DB_DIRECTORY_PATH, 'word.db')):
            os.mkdir(DEFAULT_DB_DIRECTORY_PATH)
            with open(os.path.join(DEFAULT_DB_DIRECTORY_PATH, 'word_list.txt'), 'w') as f:
                pass
